...
// ── Storage keys ─────────────────────────────────
const SK_ACTIVOS      = 'ddg_activos_v1';
const SK_CLASES       = 'ddg_clases_v1';
const SK_ASIGNACIONES = 'ddg_asig_v1';
...
// ══════════════════════════════════════════════════
function loadAll() {
  const sa = localStorage.getItem(SK_ACTIVOS);
  const sc = localStorage.getItem(SK_CLASES);
  const si = localStorage.getItem(SK_ASIGNACIONES);

  activos      = sa ? JSON.parse(sa) : structuredClone(MOCK_ACTIVOS);
  clases       = sc ? JSON.parse(sc) : structuredClone(MOCK_CLASES);
  asignaciones = si ? JSON.parse(si) : structuredClone(MOCK_ASIGNACIONES);

  if (!sa) localStorage.setItem(SK_ACTIVOS,      JSON.stringify(activos));
  if (!sc) localStorage.setItem(SK_CLASES,       JSON.stringify(clases));
  if (!si) localStorage.setItem(SK_ASIGNACIONES, JSON.stringify(asignaciones));
}

function persist() {
  localStorage.setItem(SK_ACTIVOS,      JSON.stringify(activos));
  localStorage.setItem(SK_CLASES,       JSON.stringify(clases));
  localStorage.setItem(SK_ASIGNACIONES, JSON.stringify(asignaciones));
}
